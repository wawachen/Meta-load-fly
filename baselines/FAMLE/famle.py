...
def train_meta(model, tasks_in, tasks_out, logger, meta_iter=1000, inner_iter=10, inner_step=1e-3, meta_step=1e-3, minibatch=32, inner_sample_size = None):
    '''
    ...
    for meta_count in range(meta_iter):
        ...
        batch_size = tasks_in[task_index].shape[0] if minibatch > tasks_in[task_index].shape[0] else minibatch
        ...
        for _ in range(inner_iter):
            permutation = np.random.permutation(tasks_in[task_index].shape[0]) if inner_sample_size is None else np.random.permutation(tasks_in[task_index].shape[0])[0 : min(inner_sample_size, tasks_in[task_index].shape[0])]
        ...
        if meta_count%200==0:
            model.save("/home/wawa/catkin_meta/src/MBRL_transport/FAMLE_model/model_{0}.pt".format(meta_count))

...
class CEMOptimizer(Optimizer):
    ...
    #         #costs [popsize,] store 15*[400,2] x z 
    #         costs,store_s = self.cost_function(samples, goal)
    #         eval_num = round(self.popsize*0.2)
    #         costs_p_rank = costs[np.argsort(costs)]
    #         cost_permissive = costs_p_rank[eval_num:].copy()
    #         cost_p_mean = np.mean(cost_permissive)
    #         cost_p_index = np.where(costs<cost_p_mean)[0]
    #         costs[cost_p_index] = 1000000

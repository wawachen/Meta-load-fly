...
class WindNShot:
    ...
    def __init__(self, batch_num, batchsz, path_length, n_way, k_shot, k_query, sequential=False,integrated=False):
        ...
        mat_contents_x1 = sio.loadmat("/home/wawa/catkin_meta/src/MBRL_transport/all_data/dynamics/firefly_data_3d_wind_x0.0_2agents_L0.6_dt_0.15.mat")
        ...
        mat_contents_x2 = sio.loadmat("/home/wawa/catkin_meta/src/MBRL_transport/all_data/dynamics/firefly_data_3d_wind_x0.3_2agents_L1.0_dt_0.15.mat")
        data_obs_x2 = mat_contents_x2['obs']  # [10001,4]
        data_acs_x2 = mat_contents_x2['acs']  # [10000,2]

        mat_contents_x3 = sio.loadmat("/home/wawa/catkin_meta/src/MBRL_transport/all_data/dynamics/firefly_data_3d_wind_x0.5_2agents_L0.8_dt_0.15.mat")
        data_obs_x3 = mat_contents_x3['obs']  # [10001,4]
        data_acs_x3 = mat_contents_x3['acs']  # [10000,2]

        mat_contents_x4 = sio.loadmat("/home/wawa/catkin_meta/src/MBRL_transport/all_data/dynamics/firefly_data_3d_wind_x0.8_2agents_L1.2_dt_0.15.mat")
        data_obs_x4 = mat_contents_x4['obs']  # [10001,4]
        ...
        trajs4_acs = []  
        for i in range(num_of_trajectories):
            trajs4_obs.append(data_obs_x4[(l-1)*i:(l-1)*i+l])
            trajs4_acs.append(data_acs_x4[(l-1)*i:(l-1)*(i+1)])

        new_train_in4, new_train_targs4 = [], []
        for obs, acs in zip(trajs4_obs, trajs4_acs):
            new_train_in4.append(np.concatenate([self.obs_preproc_3d(obs[:-1]), acs], axis=-1))
            new_train_targs4.append(self.targ_proc(obs[:-1], obs[1:]))
        train_in4 = np.array(new_train_in4)
        train_targs4 = np.array(new_train_targs4)

        x_all = np.concatenate([train_in1,train_in2,train_in3,train_in4], axis=0) #[trajectory_num,path length, dim]
        y_all = np.concatenate([train_targs1,train_targs2,train_targs3,train_targs4], axis=0)
    ...
    def load_data_cache(self, data_pack, data_pack_label):
        ...
        """
        #  take 4 way 1 shot as example: 4 * 1
        setsz = self.k_shot * self.n_way
        querysz = self.k_query * self.n_way
        data_cache = []

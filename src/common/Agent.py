...
class Agent:
    ...
    def sample(self, horizon, policy, wind_test_x,wind_test_y, L, adapt_size=None, log_data=None, data_path=None): 
        ...
        for t in range(horizon):
            ...
            pred, gt = policy._validate_prediction(O[t], A[t], obs)
            gt_obs.append(gt)
            pred_obs.append(pred)
        ...
        return {
            ...
            "prediction_obs": np.array(pred_obs),
            "gt_obs": np.array(gt_obs)
        }
    ...
    def embedding_sample(self, horizon, policy, wind_test_x,wind_test_y, L,log_data=None, data_path=None): 
        ...
        for t in range(horizon):
            ...
            if len(trajectory)>10:
                ...
                data_size = len(x)
                models[task_index] = train_model(model=copy.deepcopy(raw_models[task_index]), train_in=x[-data_size::], train_out=y[-data_size::], task_id=task_index)
                policy.model = models[task_index]
                ################################################
            if log_data:
                obs1 = self.env.get_uav_obs()[0]

...
class Embedding_NN(nn.Module):
    ...
    def predict_tensor(self, x, task_ids = None):
        '''Use predict methods when not optimizing the model'''
        x_normalized = (torch.Tensor(x[0,:]).cuda() - self.data_mean_input) / self.data_std_input
        if task_ids is not None:
            return (self.forward(x_normalized, task_ids) * self.data_std_output + self.data_mean_output).detach()
        else:
            tensor_shape = (x_normalized.size(0),1)
...
def load_model(file_path, device=torch.device('cpu')):
    ...
    model = Embedding_NN(**model_data["kwargs"]).cuda()
    model.load_state_dict(model_data["state_dict"])
    model.data_mean_input = model_data["others"]["data_mean_input"].cuda()
    model.data_std_input= model_data["others"]["data_std_input"].cuda()
...
def train_meta(model, cfg, tasks_in, tasks_out, logger, path):
    '''
    ...
    for task_id in range(len(tasks_in)):
        for i in range(tasks_in[task_id].shape[0]):
    ...
    for meta_count in range(cfg.meta_iter):
        weights_before = deepcopy(model.state_dict())
        task_index = int(meta_count % len(tasks_in))
        batch_size = tasks_in[task_index].shape[0] if cfg.minibatch > tasks_in[task_index].shape[0] else cfg.minibatch
        ...
        for _ in range(cfg.inner_iter):
            permutation = np.random.permutation(tasks_in[task_index].shape[0]) if cfg.inner_sample_size is None else np.random.permutation(tasks_in[task_index].shape[0])[0 : min(cfg.inner_sample_size, tasks_in[task_index].shape[0])]
            ...
            for i in range(0, x.size(0) - batch_size + 1, batch_size):    
                ...
                for param in model.parameters():
                    param.data -= cfg.inner_step * param.grad.data
        ...
        stepsize = cfg.meta_step * (1 - meta_count / cfg.meta_iter) # linear schedule
        ...
        print("Iter " + str(meta_count) + " | All task Loss: " + str(task_losses.tolist()))
        logger.add_scalar('task loss1', task_losses.tolist()[0], meta_count)
        logger.add_scalar('task loss2', task_losses.tolist()[1], meta_count)
        logger.add_scalar('task loss3', task_losses.tolist()[2], meta_count)
        logger.add_scalar('task loss4', task_losses.tolist()[3], meta_count)

        p1 = path+"/FAMLE_model"
        if not os.path.exists(p1):
            os.makedirs(p1)

        if meta_count%cfg.save_model_freq==0:
            model.save(path+"/FAMLE_model/model_{0}.pt".format(meta_count))
...
def process_data(data):
    ...
    for d in data:
        s = d[0].reshape(1,-1)
        a = d[1].reshape(1,-1)
        training_in.append(np.concatenate((obs_preproc_3d(s), a),axis=-1)[0])

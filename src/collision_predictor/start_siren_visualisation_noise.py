...
def eval():
    ...
    data_filepath_1 = "/home/wawa/catkin_meta/src/MBRL_transport/new_correction/noise_analysis/preprocess/validation"
    test_data_filepath = [data_filepath_1] # data_filepath1_train,data_filepath5,data_filepath6
    # get test file ids
    filelist = []
    ...
    ax_titles = [r'$\alpha = 0.01$', r'$\alpha = 0.03$', r'$\alpha = 0.05$','Ground Truth']
    ...
    sdf_values1 = samples[:, 2].clone()
    sdf_off_index1 = np.where(sdf_values1>0.01)[0]
    sdf_on_index1 = np.where(sdf_values1<=0.01)[0]
    sdf_values1[sdf_on_index1] = 1
    sdf_values1[sdf_off_index1] = 0
    end = time.time()
    print("sampling takes: %f" % (end - start))
    sdf_values_np1 = sdf_values1.detach().cpu().numpy()
    sdf_values_im1 = sdf_values_np1.reshape(N, N)
    img_1 = sdf_values_im1
    
    axes[0].set_axis_off()
    axes[0].imshow(img_1)
    axes[0].set_title(ax_titles[0], fontsize=25)

    sdf_values2 = samples[:, 2].clone()
    sdf_off_index2 = np.where(sdf_values2>0.03)[0]
    sdf_on_index2 = np.where(sdf_values2<=0.03)[0]
    sdf_values2[sdf_on_index2] = 1
    sdf_values2[sdf_off_index2] = 0
    end = time.time()
    print("sampling takes: %f" % (end - start))
    sdf_values_np2 = sdf_values2.detach().cpu().numpy()
    sdf_values_im2 = sdf_values_np2.reshape(N, N)
    img_2 = sdf_values_im2
    ...
    # print(sdf_values)
    sdf_values3 = samples[:, 2].clone()
    sdf_off_index3 = np.where(sdf_values3>0.05)[0]
    sdf_on_index3 = np.where(sdf_values3<=0.05)[0]
    sdf_values3[sdf_on_index3] = 1
    sdf_values3[sdf_off_index3] = 0
    end = time.time()
    print("sampling takes: %f" % (end - start))
    sdf_values_np3 = sdf_values3.detach().cpu().numpy()
    sdf_values_im3 = sdf_values_np3.reshape(N, N)
    img_3 = sdf_values_im3
